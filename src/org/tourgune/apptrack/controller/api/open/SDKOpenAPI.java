package org.tourgune.apptrack.controller.api.open;
...
import org.tourgune.apptrack.bean.PuntoList;
import org.tourgune.apptrack.bean.Valor;
import org.tourgune.apptrack.bean.ValorList;
import org.tourgune.apptrack.bean.Variable;
import org.tourgune.apptrack.facade.SDKFacade;
...
public class SDKOpenAPI {
		...
		@RequestMapping(value = "value/addemo", method = RequestMethod.POST)
		public @ResponseBody String createValueEmo(@RequestBody Emotions emo,
					@RequestParam(value = "devToken", required = true)String devToken, 
					@RequestParam(value = "appToken", required = true)String appToken){
	    	...
	    	if (idApp != -1){
	    		int idUser = sdkFacade.existUser(emo.getUser(), idApp);
	    		if ( idUser == -1){
//	    			idUser = sdkFacade.createUserEmo(emo.getUser(), idApp);
	    			return "-2";
	    		}
	    		
	    		return Integer.toString(sdkFacade.createValueEmo(emo));
	    	}
...
//						else
		 ...
		 * @return 1 si el punto es insertado correctamente, -1 en caso contrario
		 */
		@RequestMapping(value = "point/addemo", method = RequestMethod.POST, consumes = "application/json")
		public @ResponseBody Integer createPoint(@RequestBody Punto point,
				@RequestParam(value = "devToken", required = true)String devToken, 
				@RequestParam(value = "appToken", required = true)String appToken){
		...
		int idApp = sdkFacade.login(devToken, appToken);
	    	...
	    	if (idApp != -1){
	    		int idUser = sdkFacade.existUser(point.getIdusuario(), idApp);
	    		if ( idUser == -1){
//	    			idUser = sdkFacade.createUserEmo(point.getIdusuario(), idApp);
	    			return -2;
	    		}
	    		return sdkFacade.createPoint2(point, point.getIdusuario());    	
	    	}
		...
		public @ResponseBody Integer updateConectivity(@RequestBody String conexion,
	    	...
	    	if (idApp != -1){
	    		...
	    		int idUser = sdkFacade.existUser(Integer.parseInt(imei), idApp);
	    		if ( idUser == -1){
	    			return -2;
	    		}
