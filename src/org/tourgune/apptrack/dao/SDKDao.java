package org.tourgune.apptrack.dao;
...
import org.tourgune.apptrack.bean.Emotions;
import org.tourgune.apptrack.bean.Punto;
import org.tourgune.apptrack.bean.Valor;
import org.tourgune.apptrack.bean.Variable;
import org.tourgune.apptrack.dao.rowmapper.VariableRowMapper;
import org.tourgune.apptrack.utils.TablasBD;
...
StringBuffer sql = new StringBuffer();
	...
	public int createPoint2(final Punto point, final int idUser) { 
		...
		try {
    		...
    		jdbcTemplate.update(new PreparedStatementCreator() {
    			public PreparedStatement createPreparedStatement(Connection connection) throws SQLException {
    				...
    				ps.setInt(1, idUser);
    				ps.setDouble(2, point.getLatitud());
    				ps.setDouble(3, point.getLongitud());
    				ps.setTimestamp(4, Timestamp.valueOf(point.getFecha()));
    				ps.setString(5, point.getProvider());
    		...
    		lastId = (Integer)claves.get("idpunto");
		  } catch (Exception e) {
              e.printStackTrace();
       } 	
		return lastId;
	...
	public int existUser(int userId, int idApp){
		try {
			StringBuffer sql = new StringBuffer();
			
			sql.append("SELECT " + TablasBD.TABLE_USERS_ID + " ");
			sql.append("FROM " + TablasBD.TABLE_USERS + " ");
			sql.append("WHERE " + TablasBD.TABLE_USERS_ID + "= ? and " + TablasBD.TABLE_USERS_APP + " = ?");
	
			Object[] parametros;
			parametros = new Object[] {userId, idApp};
...
public int createUserEmo(final int idUser, final int idApp) { 
        ...
        try {
        	sql.append("INSERT INTO " + TablasBD.TABLE_USERS + " ");
    		sql.append("(" + TablasBD.TABLE_USERS_ID + "," + TablasBD.TABLE_USERS_APP + "," + TablasBD.TABLE_USERS_IMEI + ") ");
    		sql.append("VALUES ");
    		sql.append("(?, ?, ?)");
	...
	public int createValue(Valor value, int idUser) { 
		...
		sql.append("(" + TablasBD.TABLE_VALUES_PARAM + "," + TablasBD.TABLE_VALUES_VALUE + "," + TablasBD.TABLE_VALUES_USER +  "," + TablasBD.TABLE_VALUES_DATE +  ") ");
		sql.append("VALUES ");
		sql.append("(?, ?, ?, ?)");
		 
		Object[] parametros;
		int[] types;
		parametros = new Object[] { value.getIdvariable(), value.getValorvariable(), idUser, value.getFecha() };
		types = new int[] { Types.INTEGER, Types.VARCHAR, Types.INTEGER, Types.TIMESTAMP };
...
public int createValueEmo(Emotions emo) { 
		
		StringBuffer sql = new StringBuffer();
				
		sql.append("INSERT INTO emociones ");
		sql.append("(idusuario, pleasure, arousal, dominance, fecha, idpunto ) ");
		sql.append("VALUES ");
		sql.append("(?, ?, ?, ?, ?, ?)");
		 
		Object[] parametros;
		int[] types;
		parametros = new Object[] { emo.getUser(), emo.getPleasure(), emo.getArousal(), emo.getDominance(), emo.getDate(), emo.getIdpunto()};
		types = new int[] { Types.INTEGER, Types.INTEGER, Types.INTEGER, Types.INTEGER,Types.TIMESTAMP, Types.INTEGER };
